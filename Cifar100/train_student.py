...
def parse_option():
    ...
    # dataset
    parser.add_argument(
        "--dataset", type=str, default="cifar100", choices=["cifar100"], help="dataset"
    )
    ...
    parser.add_argument(
        "--path_s", type=str, default=None, help="student model path (optional)"
    )
    ...
    parser.add_argument(
        "-r", "--gamma", type=float, default=1, help="weight for classification"
    )
    parser.add_argument(
        "-a", "--alpha", type=float, default=None, help="weight balance for KD"
    )
    parser.add_argument(
        "-b", "--beta", type=float, default=None, help="weight balance for other losses"
    )
    # KL distillation
    parser.add_argument(
    ...
    parser.add_argument(
        "--nce_k", default=16384, type=int, help="number of negative samples for NCE"
    )
    parser.add_argument(
        "--nce_t", default=0.07, type=float, help="temperature parameter for softmax"
    )
    parser.add_argument(
        "--nce_m", default=0.5, type=float, help="momentum for non-parametric updates"
    )
...
def get_teacher_name(model_path):
    ...
    if filename.startswith("wrn"):
        ...
        return (
            filename.split("_")[0] if "x4" not in filename else filename.split("_")[0]
        )
